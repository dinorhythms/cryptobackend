...
const { badPlanId, amountOutOfRange } = messages;
...
export const processInvestment = async (req, res, next) => {
	...
	try {
		...
		if (amount < plan.minimum || amount > plan.maximum)
			return errorResponse(res, 400, "error", amountOutOfRange);
		...
		req.investment = {
			...
			endTime: new Date(
				endDate.setDate(startDate.getDate() + +plan.investmentTime)
			).toISOString(),
			status: "pending",
			createdAt: new Date().toISOString(),
			updatedAt: new Date().toISOString(),
			payToAccount:
				account[Math.floor(Math.random() * Math.floor(account.length))]
		};
